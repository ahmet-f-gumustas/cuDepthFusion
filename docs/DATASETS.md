...
## Synthetic scenes (generated, no download)

`python scripts/make_synthetic.py --suite all --output data/synthetic` writes the
analytic scenarios of `cudepthfusion.synthetic` in the same layout, plus oracle masks.
The data is generated by this repository's code (Apache-2.0) and uses no third-party
assets. Conventions, scenarios and validation results are in [SYNTHETIC.md](SYNTHETIC.md).
